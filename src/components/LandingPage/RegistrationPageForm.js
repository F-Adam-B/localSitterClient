...
export class RegistrationPageForm extends React.Component {
	onSubmit(values) {
		...
		return this.props.dispatch(registerUser(user)).then(() => this.props.dispatch(login(email, password)));
	}
	...
	render() {
		...
		if (this.props.submitSucceeded) {
			successMessage = <div className="signup sigup-success">Signed up successfully</div>;
		}

		let errorMessage;
		if (this.props.error) {
			errorMessage = <div className="signup signup-error">{this.props.error}</div>;
		}
